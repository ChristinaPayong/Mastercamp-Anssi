...
# -- VIZ 7 : Heatmap corrélation --
cells.append(nbf.v4.new_markdown_cell("### 3.7 Heatmap des corrélations"))
cells.append(nbf.v4.new_code_cell("""\
cols_num = ["cvss_score", "epss", "annee", "mois"]
corr = df_enrichi[cols_num].corr()
...
plt.tight_layout()
plt.savefig("viz_heatmap.png", bbox_inches="tight")
plt.show()
"""))

# -- VIZ 8 : Boxplot CVSS par sévérité --
cells.append(nbf.v4.new_markdown_cell("### 3.8 Boxplot des scores CVSS par sévérité"))
cells.append(nbf.v4.new_code_cell("""\
ordre = ["LOW", "MEDIUM", "HIGH", "CRITICAL"]
df_box = df_enrichi.dropna(subset=["base_severity", "cvss_score"])
df_box = df_box[df_box["base_severity"].isin(ordre)]

fig, ax = plt.subplots(figsize=(9, 5))
sns.boxplot(data=df_box, x="base_severity", y="cvss_score",
            order=ordre, palette=["#2ecc71","#f39c12","#e67e22","#e74c3c"],
            ax=ax)
ax.set_title("Distribution des scores CVSS par niveau de sévérité")
ax.set_xlabel("Sévérité")
ax.set_ylabel("Score CVSS")
plt.tight_layout()
plt.savefig("viz_boxplot.png", bbox_inches="tight")
plt.show()
"""))
...
ax.bar(features, importances, color=["#e74c3c", "#3498db"])
...
for i, (feat, imp) in enumerate(zip(features, importances)):
    ax.text(i, imp + 0.005, f"{imp:.3f}", ha="center", fontweight="bold")
plt.tight_layout()
plt.savefig("viz_importance.png", bbox_inches="tight")
plt.show()
"""))
